...
| B02 | Enter `Esc`, wait several seconds, then complete actions and nested prefixes. Repeat with `Ctrl-K`; after each Action exit, navigate, attempt a rejected edit, then make one successful edit. | The action layer never times out and command characters never leak into the buffer. Navigation/rejected edits do not cue; the first successful mutation alone shows one steady `EDIT*`/cursor acknowledgement and subsequent typing does not pulse. | |

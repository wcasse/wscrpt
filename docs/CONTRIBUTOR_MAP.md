...
| **Agent run (W2 partial)** | `src/agent_runtime.rs`, `src/agent_pi.rs`, `src/agent_acp.rs`, `src/agent.rs`; keys in `keymap` / `App` | Fake default; `profile=pi` → Pi RPC + gate; else ACP argv. `Esc w a` / `x` / `D`. **AGENTS lane.** |

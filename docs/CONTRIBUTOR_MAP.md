...
| **Agent run (W2 partial)** | `src/agent_runtime.rs`, `src/agent.rs`; keys in `keymap` / `App` | Fake plan-first loop; `Esc w a` run, `Esc w x` cancel, `Esc w D` dashboard. ACP process not launched yet. **AGENTS lane.** |

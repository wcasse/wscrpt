...
| **Agent run (W2 partial)** | `src/agent_runtime.rs`, `src/agent_acp.rs`, `src/agent.rs`; keys in `keymap` / `App` | Fake by default; ACP when `use_fake = false`. Keys: `a` run · `A` approve · `G` review handoff · `x` cancel · `D` dashboard. **AGENTS lane.** |

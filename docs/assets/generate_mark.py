...
def draw_esc_w(size: int) -> Image.Image:
    ...
    d.line(pts, fill=CYAN, width=stroke, joint="miter")
    font_size = int(200 * s)
    font = None
    for path in (
...
"""
    ...
    d.text((x0, 300), "Terminal IDE for real hosts —", font=tag_f, fill=FG)
    d.text((x0, 340), "made for iPad + Blink.", font=tag_f, fill=CYAN)
    d.text((x0, 420), "github.com/wcasse/wscrpt", font=url_f, fill=MUTED)
    d.rectangle([x0, 180, x0 + 80, 186], fill=CYAN)
    og.save(OUT / "og-1200x630.png")
    print("wrote marks + og into", OUT)

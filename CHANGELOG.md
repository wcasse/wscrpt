...
- **Agent review handoff:** on Review (or job finish in Review), open existing Git status (and the single touched-path diff when known). Manual re-run via `Esc w G` / `:agent-review`. No new VCS UI.

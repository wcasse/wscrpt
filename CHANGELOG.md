...
- Agent run loop (W2 partial): plan-first fake agent with work-packet scope, cancel, and review handoff messaging. `Esc w a` / `:agent` starts a goal, `Esc w x` cancels. Default `agent.use_fake = true`.
- **ACP process wire (minimal):** when `agent.use_fake = false` and host readiness passes, spawn `agent.argv` (e.g. `["grok", "agent", "stdio"]`), run `initialize` → `session/new` → `session/prompt`, map plan/tool updates into the Agents dashboard receipt, and kill the process group on cancel (`src/agent_acp.rs`).
- **Needs You permissions:** ACP `session/request_permission` pauses the run, shows options on the Agents dashboard, and accepts `Y` allow / `N` deny (or `Esc w A` allow). Replies over JSON-RPC; cancel maps to ACP `cancelled`.
- **Dirty-tree gate:** agent start refuses unsaved dirty buffers (save first); when Git status is ready and the worktree has changes, confirm with `Y` to start anyway or `Esc` to cancel.
- **Richer ACP mapping:** tool_call locations / raw_input paths become `path_touched` receipt lines (workspace-relative); streaming message/thought chunks coalesce into sparse dashboard notices.
- **Agent review handoff:** on Review (or job finish in Review), open existing Git surfaces only when useful (single path → diff; multi-path/dirt → status; clean+empty → status line only). Manual re-run via `Esc w G` / `:agent-review`. No new VCS UI.
- **Agent poll throttle:** progress events coalesce to one status line per batch and share the background paint budget (~15 fps); Needs You / Review / finish still paint immediately.
- **Agent dashboard paint cache:** rebuild receipt/header lines only when run state, receipt sequence, permission prompt, or panel height changes — skip `format_receipt_lines` on steady frames.
- **ACP channel backpressure:** soft events (progress/notices) `try_send` and drop when the UI is behind; hard events (Finished, Permission, Review/NeedsYou) wait briefly for a slot and abort if cancelled — never blocks kill on soft noise. Sequence numbers only advance when an event is queued.
